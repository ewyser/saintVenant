@views function advSolve_D(cublocks,cuthreads,h,Qx,Qy,UFS,Ubc,U,zbc,z,g,Δx,Δy,Δt,nx,ny,type)
    ...
    # assembly of conservative variables vector and flux function vector
        #@cuda blocks=cublocks threads=cuthreads getUh_D(U,h,Qx,Qy,nx,ny,2)
    ...
    # update along x-direction
        @cuda blocks=cublocks threads=cuthreads U!_D(h,Qx,Qy,U,UFS,(Δt/Δx),nx,ny,1)
    synchronize()
    ...
    # update along y-direction
        @cuda blocks=cublocks threads=cuthreads U!_D(h,Qx,Qy,U,UFS,(Δt/Δy),nx,ny,2)
    synchronize()
end