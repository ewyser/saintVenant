@views function advSolve_D(cublocks,cuthreads,h,Qx,Qy,UFS,Ubc,U,zbc,z,g,Δx,Δy,Δt,nx,ny,type)
    for dim ∈ 1:2 # dim ∈ [x,y]
        ...
        if type=="Rus"
            @cuda blocks=cublocks threads=cuthreads Rus!_D(UFS,Ubc,zbc,g,nx,ny,dim)
        elseif type=="HLL"
            @cuda blocks=cublocks threads=cuthreads HLL!_D(UFS,Ubc,zbc,g,nx,ny,dim)
        elseif type=="HLLC"
            @cuda blocks=cublocks threads=cuthreads HLLC!_D(UFS,Ubc,zbc,g,nx,ny,dim)
        else 
        ...
        @cuda blocks=cublocks threads=cuthreads U!_D(h,Qx,Qy,U,UFS,(Δt/Δx),nx,ny,dim)
        synchronize()
    end
end